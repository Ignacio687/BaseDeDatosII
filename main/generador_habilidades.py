...
tipos_req_arma = ["A distancia","A dos manos","A una mano", "Arma Cuerpo a Cuerpo", "Arma Magica"]
...
def generar_registro(key,element):
    return {
        ...
        "detalle": element,
        "efecto": random.choice(bonificaciones),
        "requerimiento": [
            {"tipo": random.choice(tipos_req_arma)},
            {random.choice(tipos_req_habilidades): random.randint(1, 11)}
        ]
    }
